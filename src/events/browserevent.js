...
define([
  ...
  function(util, BrowserFeature, EventBase, EventType, UA) {
    ...
    util.mixin(BrowserEvent.prototype, {
      ...
      /**
       * IE只有keyCode属性,表示按键的ascii码.onkeydown会在任何情况下触发,
       * 但是keypress对于功能键,后退键和方向键失效. keypress中的keyCode对应
       * ascii码正常,keydown中的对应了大写字母表或者一个虚拟键码.一般来讲
       * keypress得到的keycode符合预期.
       * @type {number}
       */
      keyCode: 0,
       ...
       * History事件对象, 只在PopState事件出现, 是pushState或者replaceState
       * 时提供的状态对象的一个copy.
       * @type {Object}
       */
      state: null,
      ...
      init: function(evt, opt_currentTarget) {
        ...
        this.offsetX = (UA.isWEBKIT || evt.offsetX !== undefined) ?
          evt.offsetX : evt.layerX;
        this.offsetY = (UA.isWEBKIT || evt.offsetY !== undefined) ?
